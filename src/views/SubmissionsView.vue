      ...
      <div class="card-body p-0">
        ...
        <div v-else class="table-responsive">
          <table class="table table-hover mb-0">
            ...
            <tbody>
              <tr v-for="s in subStore.submissions" :key="s._id" :class="{ 'table-active': selectedSubIds.includes(s._id), 'table-success': s.status === 'Approved' && !s.printed, 'table-danger': s.status === 'Rejected' }">
                <td>
                  <input
                    v-if="s.status === 'Draft'"
                    type="checkbox"
